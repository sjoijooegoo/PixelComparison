...
def resolve_heat_scales(
    ...
    *,
    ...
    if binding:
        ...
        configured = {
            row["assigned_metric_key"]: configured_scale_dto(
                {**dict(row), "segments": json.loads(row["segments_json"])}, binding["scale_set_id"], binding["name"],
            )
            for row in rows
        }

    return resolve_metric_scales(heat_map, points, configured)
