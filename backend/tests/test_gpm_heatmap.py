...
def _report(*, point_key=None, embedded_metadata=False):
    point = {
        "index": 1,
        "screenshot_id": 1,
    ...
    scene = {
        ...
        "trend": [{"key": "Scene_DC", "name": "Scene_DC", "index": 0}],
        "detail": [point],
    }
...
def _upload(
    ...
    return client.post(
        ...
        files={
            "report": ("GPMHeatmap.json", json.dumps(_report(point_key=point_key)).encode(), "application/json"),
