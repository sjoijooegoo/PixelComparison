...
GPM 初始化不仅检查 schema 版本，还核对表/列指纹。全新路径会创建当前唯一 schema；已有数据库一旦不兼容则记录数据库路径和版本、拒绝启动，并保持数据库、WAL 与资源目录原样。它不会运行历史兼容分支，也绝不能把结构不匹配解释为删除数据的授权。修改 GPM schema 或把现有库接入新代码前，必须先备份数据库与 `assets`，并通过受控的离线流程转换或显式使用空数据目录。
...
| 历史批次深链 | URL 带 batch 就一定固定历史批次 | 烘培与 GPM 新进入页面优先最新；截图角色则恢复可见合法运行。烘培页内遇到已删除或滑出当前日期窗口的批次会回退到窗口内最新批次并规范化 URL；属于其他场景/分支/平台/画质的有效批次仍拒绝跨范围读取。三页语义不同 |
...
| 烘培拓扑/指标/趋势 | `test_map_build_data.py` | `views/MapBuildView.test.js`、`mapBuildRoute.test.js`、`mapBuildPresentation.test.js`、`components/MapBuildTrendChart.test.js` | 自身/含子级；无索引反射分块；缺节点断线；同日多点；失效批次回退；60 天边界 |
