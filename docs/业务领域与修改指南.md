...
| GPM 单点历史 | 稳定 `point_key`，限定分支、地图、平台、画质 | 点位序号、截图名、相近坐标 |
...
GPM 接收一次完整的报告 JSON 和截图 ZIP；一个批次可以包含多个地图，但每个地图的 P4、平台和画质必须一致。地图名不能重复。点位包括序号、截图标识、位置、方向、热力指标、趋势指标和详情树；`point_key` 可选，用于稳定的单点历史关联。
...
地图配置是跨批次、跨分支共享的资源。报告出现一个未知地图，不会自动把它创建为可用地图配置。批次管理仍可显示这类上报及配置缺失状态。
...
同一地图、同一平台内切换批次或画质时，只能用稳定 `point_key` 恢复相同逻辑点；没有稳定 Key 时回到新帧首点，不能用截图 ID 或序号猜测。切换平台或地图不继承点位选择。
...
单点趋势读取同作用域、同 `point_key` 的 `trend_data`。没有稳定 Key 时标记历史不可用，只返回当前点，而不是把不同批次的相同序号连起来。趋势缺失值应断线，不能变成 0。
...
| GPM 地图/颜色/详情/趋势 | `test_gpm_heatmap.py`、`test_gpm_scale_expressions.py` | `gpmHeatmap/*.test.js`；`components/` 下的 `GpmMapCanvas.test.js`、`GpmDetailNode.test.js`、`GpmDetailPanel.test.js`、`GpmTrendCard.test.js`、`GpmScreenshotStrip.test.js` | 轴向与留白；边界区间；未知 Key 回退；稳定 point_key；详情状态与迟到响应 |
