...
在热力图工作区点击顶栏“批次管理”，会沿用当前显示批次的分支、地图、平台和画质，自动打开该批次所在页并高亮该行。日期范围默认最近 30 天；来源时间超出范围时会扩展日期，保证来源可见。

列表 URL 的 `focus_batch` 保存定位目标。页码按当前筛选、入库 ID 降序和每页数量计算；刷新或窗口改变每页行数时会重新定位。手动翻页、修改筛选或清空条件会取消定位。来源批次已删除或不在筛选范围时提示原因并回到第一页，不阻止继续浏览列表。

批次列表接口 `GET /api/gpm-heatmaps/uploads` 可选传 `locate_batch_id`，成功时返回目标所在的 `page` 和 `located_batch_id`；目标不在当前筛选内时返回第一页和 `located_batch_id: null`，不会扩大后端查询范围。

### 导出批次到离线查看器

在“批次管理 → 热力图批次”的操作列点击“导出”，下载该批次的 `.ssheat` 离线包。浏览器按下载设置保存文件；把文件复制到离线查看器的 `data/` 目录，或在保存窗口直接选择该目录，再刷新离线页面即可追加批次并重建趋势。

批次包只包含点位、截图缩略图、详情和趋势数据，复用查看器已有的 `config/heatmap.zip`。新批次包含尚未配置的地图，或地图/标尺配置发生变更时，需另外导出“全部配置”，保存为 `config/heatmap.zip`。重复导出同一批次应替换旧文件，不要同时保留带 `(1)` 等后缀的重复包。目录和配置说明见[离线查看包方案](热力图离线查看包方案调研.md)。
...
GPMHeatmap 只存在一份最终 schema，不提供运行时历史迁移或兼容表。后端检测到已有独立 GPM 数据库与当前表、列或版本指纹不一致时，会输出数据库路径、实际版本和期望版本并拒绝启动；数据库、WAL 文件和 GPM 资源目录均保持原样。部署涉及 schema 变化的版本前，必须先备份数据库与 `assets`，再通过受控的离线流程转换，或显式改用空数据目录。
